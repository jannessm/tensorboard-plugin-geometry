...
for logs in ['./logs/test_plugin', './logs/another_test', './logs/a_good_one']:
  ...
  for i in range(10):
    writer.add_geometry('test_geo', pos.reshape(1, 100, 3), features=i*wss.reshape(1, 100, 3), global_step=i)
    writer.add_geometry('test_geo_2', i*pos.reshape(1, 100, 3), features=wss.reshape(1, 100, 3), global_step=i)
